...
% Parse inputs
defaults = struct('subjects_dir','/usr/local/freesurfer/subjects', 'layers_n', 11);  %define default values
...
hemispheres={'lh','rh'};
% layers=[0.1:0.1:0.9];
output_name = strcat("multilayer-", num2str(params.layers_n), ".ds.gii")
layers = linspace(0.1, 0.9, params.layers_n)
  
for l=1:length(layers)
    layer=layers(l);
    for h=1:length(hemispheres)
        hemi=hemispheres{h};
        wm_file=fullfile(params.subjects_dir, 'surf', sprintf('%s.white',hemi));
        out_file=fullfile(params.subjects_dir, 'surf', sprintf('%s.%.1f',hemi,layer));
...
for l=1:length(layers)
    ...
    for h_idx=1:length(hemispheres)    
        ...
        orig_name=fullfile(params.subjects_dir,...
            'surf', sprintf('%s.%.1f', hemi, layer));
        new_name=fullfile(params.subjects_dir,...
    ...
    % combine hemispheres
    lh=fullfile(params.subjects_dir, 'surf', sprintf('lh.%.1f.gii', layer));
    rh=fullfile(params.subjects_dir, 'surf', sprintf('rh.%.1f.gii', layer));
    combined=fullfile(params.subjects_dir, 'surf', sprintf('%.1f.gii', layer));
...
combined=fullfile(params.subjects_dir, subj_id, 'surf', output_name);
